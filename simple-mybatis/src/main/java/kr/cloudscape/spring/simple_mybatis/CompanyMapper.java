...
public interface CompanyMapper {
    ...
    @Results(id = "CompanyMap", value = {
            ...
            @Result(property = "employeeList",
                    column = "id",
                    many = @Many(select = "kr.cloudscape.spring.simple_mybatis.EmployeeMapper.getByCompanyId"))  // Subquery를 이렇게 어노테이션을 통해 처리.
    })  // Company의 property 이름과 테이블의 컬럼명이 틀리기 때문에 이렇게 mapping이 필요하다.
        // company.id 의 경우 Company 오브젝트에서도 이름이 같으므로 필요 없다.
    List<Company> getAll();
